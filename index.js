...
//----------------------------Traitement d'ical---------------------------------------
function update() {
	request(icsLink, function (err, res, body) {
		if (err != null) {
			console.error(err);
			return -1;
		}
		...
		try {
			const data = fs.writeFileSync("result/tdTp.ics", tdTp);
		...
		try {
			const data = fs.writeFileSync("result/amphi.ics", amphi);
			//file written successfully
		} catch (err) {
			console.error(err);
		}
		...
		try {
			const data = fs.writeFileSync("result/controle.ics", controle);
			//file written successfully
		} catch (err) {
			console.error(err);
		}
